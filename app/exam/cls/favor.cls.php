...
class favor_exam
{
	...
	public function getAllExamHistoryListByArgs($page,$number = 20,$args = array(),$fields = false,$order=0)
	{
		$args[] = array("AND","examhistory.ehuserid = user.userid");
		$page = $page > 0?$page:1;
		$r = array();
		if($order==0){
			$data = array($fields,array('examhistory','user'),$args,false,"usergroupid ASC,ehuserid ASC",array(intval($page-1)*$number,$number));
		}
		if($order==1){
			$data = array($fields,array('examhistory','user'),$args,false,"ehscore DESC",array(intval($page-1)*$number,$number));
		}
		if($order==2){
			$data = array($fields,array('examhistory','user'),$args,false,"ehscore ASC",array(intval($page-1)*$number,$number));
		}
		if($order==3){
			$data = array($fields,array('examhistory','user'),$args,false,"ehtime DESC",array(intval($page-1)*$number,$number));
		}
		if($order==4){
			$data = array($fields,array('examhistory','user'),$args,false,"ehtime ASC",array(intval($page-1)*$number,$number));
		}
		$sql = $this->pdosql->makeSelect($data);
		$r['data'] = $this->db->fetchAll($sql,'ehid');
	...
	{
		$args[] = array("AND","examhistory.ehuserid = user.userid");
		//$data = array($fields,array('examhistory','user'),$args,false,"ehuserid ASC",false);
		$data = array($fields,array('examhistory','user'),$args,false,"usergroupid,userid ASC",false); //班级，学号来排序 
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetchAll($sql);
	}
