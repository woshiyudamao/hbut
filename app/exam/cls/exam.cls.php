...
class exam_exam
{
	...
	public function addExamtimeBySessionid($sessionid) //通过Sessionid增加考试时间 by yu 2015.12.10
	{
		...
		else
		{
			...
			if(!empty($Exam))
			{	$args=array();
				$args['examsessiontime']=$Exam['examsessiontime']+5;
				if($this->modifyExamSession($args,$sessionid))
				return true;	
				else
				return false;	
	...
	//参数：批量试题内容字符串，知识点ID
	//返回值：true
	public function importQuestionBat($uploadfile,$subject=0,$questionparent = 0) // 2015.12.3 by yu
	{
		...
		while ($data = fgetcsv($handle))
		{
			...
			if(count($question) >= 6)
			{	
				$questiontype=iconv("GBK","UTF-8//IGNORE",trim($question[6]," \n\t")); //获取题型 将其转换成ID类型
				if($questiontype=='单选') 		$args['questiontype'] =1;
				else  if($questiontype=='多选') $args['questiontype'] =2;
				else  if($questiontype=='判断') $args['questiontype'] =3;
				else continue; //没有符合的情况直接跳过
				$zhang	= intval(trim($question[1]," \n\t")); //章
				$jie	= intval(trim($question[2]," \n\t")); //节
				$zhishidian = $this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[5]," \n\t")))); //知识点
				if($zhishidian=='') $zhishidian='无'; //如果知识点为空的话就设置 知识点为 ”无“
				$select=array();
				if($args['questiontype']==1 || $args['questiontype']==2) //如果是单选题或者多选题的话就处理选项
				{
				$select['A']=$this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[8]," \n\t"))));
				$select['B']=$this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[9]," \n\t"))));
				$select['C']=$this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[10]," \n\t"))));
				$select['D']=$this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[11]," \n\t"))));
				$args['questionselectnumber']=0; //初始化为0
				$args['questionanswer'] = $this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[12]," \n\t"))));
				}
				else
				{ //判断题采取 A B 选项 对错
					$select['A']="对";
					$select['B']="错";
					$args['questionselectnumber']=0; //之前设置为2 忘记了下面计算了一次
					$questionanswer = $this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[12]," \n\t"))));
					if ($questionanswer =='T') $args['questionanswer'] ='A';
					else if ($questionanswer =='F') $args['questionanswer'] ='B';
				}
				$questionselect=""; //首先清空一下内容
				foreach($select as $k => $v)
				{	if(!empty($v)) //如果不为空的话
					{//计算选项个数
					$args['questionselectnumber']++;//选项加1
					$questionselect=$questionselect."<p>".$k."、".$v."</p>"; //展开选项
					}
				}
				$args['question'] = $this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[7]," \n\t")))); //题干部分
				$args['questionselect'] =$this->ev->addSlashes(htmlspecialchars($questionselect)); // 选项部分
				$args['questiondescribe'] = ''; //这是解析 不过所提供的的题库并没有解析
				///////////////////////////////// 计算所属知识点
				$section = $this->section->getSectionByArgs(array(array("AND","section = :section",'section',$zhang.$jie),array("AND","sectionsubjectid = :sectionsubjectid",'sectionsubjectid',$subject))); //马原ID 为4
				if($section) //如果已经存在了就直接获取ID
				$sectionid=$section['sectionid'];
				else //不存在的话就先添加之后在获取ID
				{	$args_section=array();
					$args_section['section']= $zhang.$jie;
					$args_section['sectionsubjectid']= $subject ;
					$sectionid=$this->section->addSection($args_section);
			
				}
				
						$knows = $this->section->getKnowsByArgs(array(array("AND","knowsstatus = 1"),array("AND","knows = :knows",'knows',$zhishidian),array("AND","knowssectionid = :knowssectionid",'knowssectionid',$sectionid)));
						if($knows )
						$tknowsid=$knows['knowsid'];
						else
						$tknowsid=$this->section->addKnows(array("knowssectionid" => $sectionid,"knows" => $zhishidian));
				...
				if($questionparent)$args['questionparent'] = $questionparent; //这个暂时还不了解是什么
				$questionlevel=iconv("GBK","UTF-8//IGNORE",trim($question[13]," \n\t"));
				if($questionlevel=='易') 	   $args['questionlevel'] =1;
				else  if($questionlevel=='中') $args['questionlevel'] =2;
				else  if($questionlevel=='难') $args['questionlevel'] =3;
				$args['questioncreatetime'] = TIME;
				$args['questionuserid'] = 1; //录入人ID
				$args['questionusername'] = $this->ev->addSlashes(htmlspecialchars(iconv("GBK","UTF-8//IGNORE",trim($question[14]," \n\t")))); //录入姓名 这里与ID不符合 不知道会出现什么问题
