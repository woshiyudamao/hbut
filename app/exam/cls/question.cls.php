...
class question_exam
{
	...
	//获取试题列表 //添加不同知识点的功能 by yu 2015.12.20
	public function getRandQuestionList($args = 1,$diff_knows=false)
	{
		...
		if($diff_knows==true) //如果满足知识点不重复 就将 qkknowsid 也加入进来
		$data = array('DISTINCT questions.questionid,quest2knows.qkknowsid',array('questions','quest2knows'),$args,false,false,false);
		else
		$data = array('DISTINCT questions.questionid',array('questions','quest2knows'),$args,false,false,false);
	
		$sql = $this->pdosql->makeSelect($data);
		$r = $this->db->fetchAll($sql);
		if($diff_knows) return $r; //如果是知识点不唯一就直接跳出
		$t = array();
		foreach($r as $p)
		{
			$t[] = $p['questionid'];
	...
	//筛选随机试题 //add 知识点不重复 by yu 2015.12.20
	public function selectQuestions($examid,$basic)
	{
		$exam = $this->exam->getExamSettingById($examid);
		if($exam['examsetting']['scalemodel']) //如果开启了题量配比
		...
		$selectedknowsids=array(); //保存已经选择的知识点
		...
		foreach($settings['questype'] as $key => $p)
		{
			...
			foreach($number as $nkey => $t)
			{
				...
				if($t)
				{	//这个是获取满足所有条件的试题
					$r = $this->getRandQuestionList(array(array("AND","find_in_set(quest2knows.qkknowsid,:knowsids)",'knowsids',$knowsids),array("AND","questions.questionlevel = :questionlevel",'questionlevel',$nkey),array("AND","questions.questiontype = :questiontype",'questiontype',$key)),true);
					//首先单个题型的知识点不重复
					//然后去掉已经选出来的知识点
					//达到所有题型不重复知识点的原则
					...
					if(is_array($r))
					{
						if((count($r) >= $t)) //题库充足的情况下
						{
							$ltk=count($r); //题库长度
							for($ii=0;$ii<$t;$ii++)
							{	$repickcount=0; //重复次数 防止无限选取
								do
								{
								$repickcount++;
								if($repickcount > 200) break; //如果随机的次数比某数值还大并且还没找到不重复的就认定题库数量不够! 该数值可以人工确定
								$qtrand=rand(0,count($r)-1); //随机一个
								$repickrand=false;	
								foreach($selectedknowsids as $selectedval)
								if($r[$qtrand]['qkknowsid']==$selectedval) 
									{
									$repickrand=true; //如果有重复的知识点就重新选取
									break;
									}
								}
								while($repickrand);
								$selectedknowsids[]=$r[$qtrand]['qkknowsid'];  //保存已经选择的知识点
								$question[$key][] = $r[$qtrand]['questionid']; //添加试题
							}
							$t = 0;
						}
						else
		...
		//$myfile=fopen("qlist.txt","a");
		//fwrite($myfile,trim(implode(',',$selectedknowsids)));
		//fclose($myfile);
