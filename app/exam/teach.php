...
class app
{
	...
	public function users()
	{
		...
		switch($subaction)
		{
			...
			// }
			if($search['exampaper']){
					$args[] = array('AND',"ehexam = :esexam",'esexam',$search['exampaper']);
			    }
			if($search['groupid']){
					$args[] = array("AND","user.usergroupid = :groupid",'groupid',$search['groupid']);
			    }
			...
			case 'outspaper': //批量导出试卷 by yu 2015.12.28
			set_time_limit(0); //不超时
			$groupinfo = $this->user->getUserGroups(); //获取所有用户组
			$args = array();
			$count_sucs=0;
			$count_fail=0;
			...
			$allrs = $this->favor->getAllExamHistoryByArgs($args,array('usergroupid','ehusername','ehscore','usertruename','ehexam','ehscorelist','ehquestion','ehuseranswer'));
			foreach($allrs as $eh)
			{				
			$sessionvars = array('examsession'=>$eh['ehexam'],'examsessionscore'=>$eh['ehscore'],'examsessionscorelist'=>unserialize($eh['ehscorelist']),'examsessionquestion'=>unserialize($eh['ehquestion']),'examsessionuseranswer'=>unserialize($eh['ehuseranswer']));
			$r=array();
			$userans=$sessionvars['examsessionuseranswer'];//考生选择的答案
			$rs=$sessionvars['examsessionquestion']['questions'];
			$r[] = array('questionid'=>'ID','question' => iconv("UTF-8","GBK",'问题'),'questionselect' => iconv("UTF-8","GBK",'选项'),'questionanswer' => iconv("UTF-8","GBK",'正确答案'),'useranswer' => iconv("UTF-8","GBK",'考生答案'));
			$qcount=0; //试题计数
			foreach($rs as $p=>$k) //题型三种
			foreach($rs[$p] as $pp=>$kk)
			{			$qcount++; //试题计数加一
						if(is_array($userans[$kk['questionid']])) //如果是多项选择的话就将其展开
						$useranswer=implode('',$userans[$kk['questionid']]);
						else
						$useranswer=$userans[$kk['questionid']];	
						$r[] = array('questionid' => $kk['questionid'],
						 'question' => iconv("UTF-8","GBK",$kk['question']),
						 'questionselect' => iconv("UTF-8","GBK",$kk['questionselect']),
						 'questionanswer' => $kk['questionanswer'],
						 'useranswer' => $useranswer
						 );
			}
			$fname = 'data/papar/'.iconv("UTF-8","GBK",$groupinfo[$eh['usergroupid']]['groupname']).'/'.iconv("UTF-8","GBK",$eh['ehexam']).'/'.$eh['ehusername'].'-'.intval($eh['ehscore']).'('.$qcount.').csv'; //生成的试卷名称
			...
			$message = array(
					'statusCode' => 200,
					"message" => "成功导出 $count_sucs 张试卷，失败 $count_fail 张!",
				    "callbackType" => 'forward'
				);
			...
			$allrs = $this->favor->getAllExamHistoryByArgs($args,array('usergroupid','ehscore','ehexam','ehscorelist','ehquestion','ehuseranswer','ehtime'));
			$single=0;  //单选数量
			$sin_right=0;//单选正确率
			$multi=0;
			$mul_right=0;
			...
			$qknow_c=array();
			$qknow_rc=array();
			$temp = $allrs[0]['usergroupid'];
			$for_count=count(array_values($allrs));
			foreach($allrs as $eh)
			{
			$count_for++;
			if($temp!=$eh['usergroupid']){  //每个班
				$sin_acc=$sin_right/$single;
				$multi_acc=$mul_right/$multi;
				$jud_acc=$jud_right/$judge;
				$average=$score/$count;//平均分
				$time/=$count;   
				$time/=60; //平均时间（分钟）
					$r[] = array(
					'number' => $count,
					'time' => iconv("UTF-8","GBK",round($time,1).'分钟'),
					'highest' => $highest,
					'lowest' => $lowest,
					'average' => $average,
					'sin_count' => $single,
					'sin_acc' => round($sin_acc*100,2).'%',
					'multi_count' => $multi,
					'multi_acc' => round($multi_acc*100,2).'%',
					'jud_count' => $judge,
					'jud_acc' => round($jud_acc*100,2).'%');
					$r[] = array();
					$r[] = array();
					$r[] = array(
					'number' => iconv("UTF-8","GBK",'知识点'),
					'time' => iconv("UTF-8","GBK",'出现数量'),
					'highest' => iconv("UTF-8","GBK",'正确数量'));
					foreach($qknow as $key=>$value){
						if($qknow_rc[$key]==''){
							$qknow_rc[$key]=0;
						}
						$r[] = array(
					'number' => iconv("UTF-8","GBK",$value),
					'time' => $qknow_c[$key],
					'highest' => $qknow_rc[$key]);
					}
					$fname = 'data/analyse/'.iconv("UTF-8","GBK",$exam).'/'.iconv("UTF-8","GBK",$info).'-('.$count.iconv("UTF-8","GBK",'人)').'.csv'; //生成的试卷名称	
				if($this->files->outCsv($fname,$r))
				$count_sucs++;
				else
				$count_fail++;	
			...
			$userans=$sessionvars['examsessionuseranswer'];//考生选择的答案
			$rs=$sessionvars['examsessionquestion']['questions'];
			$r[] = array('number' => iconv("UTF-8","GBK",'考试人数'),'time' => iconv("UTF-8","GBK",'平均时间'),'highest' => iconv("UTF-8","GBK",'最高分'),'lowest' => iconv("UTF-8","GBK",'最低分'),'average' => iconv("UTF-8","GBK",'平均分'),'sin_count' => iconv("UTF-8","GBK",'单选数量'),'sin_acc' => iconv("UTF-8","GBK",'单选正确率'),'multi_count' => iconv("UTF-8","GBK",'多选数量'),'multi_acc' => iconv("UTF-8","GBK",'多选正确率')
			,'jud_count' => iconv("UTF-8","GBK",'判断数量'),'jud_acc' => iconv("UTF-8","GBK",'判断正确率'));
			foreach($rs as $p=>$k) //题型三种
			foreach($rs[$p] as $pp=>$kk)
			{		
		

				$qknow[$kk['questionknowsid'][0]['knowsid']]=$kk['questionknowsid'][0]['knows'];
				$qknow_c[$kk['questionknowsid'][0]['knowsid']]++;
			
				
				
				$useranswer=$userans[$kk['questionid']];
				if(strlen($kk['questionanswer'])>1){   //如果是多项选择的话
				$multi++;
				
					if(is_array($userans[$kk['questionid']])) 
					$useranswer=implode('',$userans[$kk['questionid']]);
					if($kk['questionanswer']==$useranswer){
					$mul_right++;
					$qknow_rc[$kk['questionknowsid'][0]['knowsid']]+=1;}
				} 
				else if(preg_match_all('</p>',$kk['questionselect'],$mat)==2){
					$judge++;
					if($kk['questionanswer']==$useranswer){
					$jud_right++;
					$qknow_rc[$kk['questionknowsid'][0]['knowsid']]+=1;}
			...
			$time+=$eh['ehtime'];
			$exam = $eh['ehexam'];  //记录上一次数据
			$info = $groupinfo[$eh['usergroupid']]['groupname'];
			$count++;
			$score+=$eh['ehscore'];
			if($eh['ehscore']>$highest)
				$highest=$eh['ehscore'];
			if($eh['ehscore']<$lowest)
				$lowest=$eh['ehscore'];
			if($count_for>=$for_count){
				$sin_acc=$sin_right/$single;
				$multi_acc=$mul_right/$multi;
				$jud_acc=$jud_right/$judge;
				$average=$score/$count;//平均分
			...
			$message = array(
					'statusCode' => 200,
					"message" => "成功导出 $count_sucs 个班级考试分析，失败 $count_fail 个!",
				    "callbackType" => 'forward'
				);
			...
			case 'outscorewithtime': //导出带时间的成绩单
			set_time_limit(0); //不超时
			$args = array();
			$groupinfo = $this->user->getUserGroups();
			$args[] = array('AND',"ehscore <= :escore",'escore', 30 );
			$rs = $this->favor->getAllExamHistoryByArgs($args,array('usergroupid','ehusername','ehscore','usertruename','ehexam','ehtime')); 
				
			foreach($rs as $pp)
			{				
						$r[] = array('usergroupname' => iconv("UTF-8","GBK",$groupinfo[$pp['usergroupid']]['groupname']),
						 'ehusername' => iconv("UTF-8","GBK",$pp['ehusername']),
						 'usertruename' => iconv("UTF-8","GBK",$pp['usertruename']),
						 'ehexam' => iconv("UTF-8","GBK",$pp['ehexam']),
						 'ehscore' => $pp['ehscore'],
						 'ehtime' => intval($pp['ehtime'] /60),
						 );
			}
			
			
			$fname = 'data/score/'.TIME.'.csv'; //生成的试卷名称
			if($this->files->outCsv($fname,$r))
				$message = array(
					'statusCode' => 200,
			...
			//{
				...
				$rs = $this->favor->getAllExamHistoryByArgs($args,array('usergroupid','ehusername','ehscore','usertruename','ehexam','ehquestion')); //加入ehquestion 计算试题个数 可能会增大内存消耗
				//添加一条结束数据
				$rs[]=array('usergroupid' => -1 ,'ehusername' => 0,'ehscore' => 0 ,'usertruename' => '','ehexam' => '','ehquestion' => array());
				$r = array();
				//exit(count($rs).'---');
				$oldusergroupid=0;
				$count_sucs=0;
				$count_fail=0;
				foreach($rs as $p) //加入计算试题个数的 少题加上分数 by yu 2015.12.29 仅仅针对这次考试
				{	
					if($oldusergroupid==0) $oldusergroupid=$p['usergroupid'];
					$questions=unserialize($p['ehquestion'])['questions'];
					$count=0;
					if($oldusergroupid!=$p['usergroupid'])
					{	
						$fname = 'data/score/'.iconv("UTF-8","GBK",$groupinfo[$oldusergroupid]['groupname']).'-score.xls';
						/*if($this->files->outCsv($fname,$r))
						{
							$count_sucs++;
							$r=array(); //成功之后要清楚记录
							$oldusergroupid=$p['usergroupid']; //要将班级复制
							$r[]=array('usergroupid' => iconv("UTF-8","GBK",'班级'),'ehusername' => iconv("UTF-8","GBK",'学号'),'usertruename' => iconv("UTF-8","GBK",'姓名'),'ehexam' => iconv("UTF-8","GBK",'科目'),'ehscore' => iconv("UTF-8","GBK",'分数'),'mark' => iconv("UTF-8","GBK",'备注'));
						...
						*/
						...
						$resultPHPExcel->getActiveSheet()->setCellValue('A1', '班级'); 
						$resultPHPExcel->getActiveSheet()->setCellValue('B1', '学号'); 
						$resultPHPExcel->getActiveSheet()->setCellValue('C1', '姓名'); 
						$resultPHPExcel->getActiveSheet()->setCellValue('D1', '科目'); 
						$resultPHPExcel->getActiveSheet()->setCellValue('E1', '分数'); 
						$resultPHPExcel->getActiveSheet()->setCellValue('F1', '备注'); 
						$i = 2; 
						foreach($r as $rr)
						
						{
							$resultPHPExcel->getActiveSheet()->setCellValue('A' . $i, iconv("GBK","UTF-8",$rr['usergroupid'])); 
							$resultPHPExcel->getActiveSheet()->setCellValue('B' . $i, $rr['ehusername']); 
							$resultPHPExcel->getActiveSheet()->setCellValue('C' . $i, iconv("GBK","UTF-8",$rr['usertruename'])); 
							$resultPHPExcel->getActiveSheet()->setCellValue('D' . $i, iconv("GBK","UTF-8",$rr['ehexam'])); 
							$resultPHPExcel->getActiveSheet()->setCellValue('E' . $i, $rr['ehscore']); 
							$resultPHPExcel->getActiveSheet()->setCellValue('F' . $i, iconv("GBK","UTF-8",$rr['mark'])); 
							$i ++; 
						}
						try{
						$xlsWriter = new PHPExcel_Writer_Excel5($resultPHPExcel); 
						$xlsWriter->save($fname);
						$count_sucs++;
						}
						catch (Exception $e) { 
						$count_fail++;	
					...
					$count=count($questions['1']) +count($questions['2']) +count($questions['3']) ; //这个计数方式仅针对这次考试 50 题 三个类型
					if($count !=50)
					{	$add_fix= (50-$count) *2;
						$p['ehscore']=intval($p['ehscore'])+$add_fix;
						$mark=iconv("UTF-8","GBK","试卷题目".$count."自动补分" .$add_fix);
					}
					else
					$mark='';	
					$r[] = array('usergroupid' => iconv("UTF-8","GBK",$groupinfo[$p['usergroupid']]['groupname']),'ehusername' => iconv("UTF-8","GBK",$p['ehusername']),'usertruename' => iconv("UTF-8","GBK",$p['usertruename']),'ehexam' => iconv("UTF-8","GBK",$p['ehexam']),'ehscore' => $p['ehscore'] ,'mark' => $mark);
				...
				$message = array(
					'statusCode' => 200,
					"message" => "已导出 $count_sucs 个班级，失败 $count_fail 个班级!",
				    "callbackType" => 'forward'
				);
			...
			case 'downpaper': //下载考生试卷及答案 2015.12.3 by yu
			$ehid = $this->ev->get('ehid');
			$eh = $this->favor->getExamHistoryById($ehid);
			//print_r($eh);
			//exit();
			$questype = $this->basic->getQuestypeList();
			$thisuser=$this->user->getUserById($eh['ehuserid']);
			$sessionvars = array('examsession'=>$eh['ehexam'],'examsessionscore'=>$eh['ehscore'],'examsessionscorelist'=>$eh['ehscorelist'],'examsessionsetting'=>$eh['ehsetting'],'examsessionquestion'=>$eh['ehquestion'],'examsessionuseranswer'=>$eh['ehuseranswer']);
			$fname = 'data/papar/'.$thisuser['username'].'-'.$ehid.'-'.TIME.'.csv'; //生成的试卷名称
			$r=array();
			$userans=$sessionvars['examsessionuseranswer'];//考生选择的答案
			$rs=$sessionvars['examsessionquestion']['questions'];
			$r[] = array('questionid'=>'ID','question' => iconv("UTF-8","GBK",'问题'),'questionselect' => iconv("UTF-8","GBK",'选项'),'questionanswer' => iconv("UTF-8","GBK",'正确答案'),'useranswer' => iconv("UTF-8","GBK",'考生答案'));
			...
			case 'scorelist':
			set_time_limit(0); //不超时
			$order = $this->ev->get('order');
			$app =array();
			$app[]=array('AND',"user.usergroupid = user_group.groupid");
			$userinfo =$this->user->getUsersByArgs($app); 
			$usergroupsmsg = $this->user->getUserGroupsASC();
			$usergroups = array_keys($usergroupsmsg);
			$examsforpaper = $this->favor->getAllExamHistoryByArgs();
			$papers = array();
			foreach($examsforpaper as $key=>$value){
				$papers[] = $value['ehexam'];
			}
            $exampapers=array_unique($papers);
			$this->tpl->assign('userinfo',$userinfo);
	        $this->tpl->assign('exampapers',$exampapers);
				
			$this->tpl->assign('usergroups',$usergroups);
			$this->tpl->assign('usergroupsmsg',$usergroupsmsg);
			$page = $this->ev->get('page');
			...
			if($order){
				$exams = $this->favor->getAllExamHistoryListByArgs($page,30,$args,false,$order);
			}else{
				$exams = $this->favor->getAllExamHistoryListByArgs($page,30,$args,false);
			}
