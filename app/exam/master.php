...
class app
{
	...
	public function basic()
	{
		...
		switch($subaction)
		{
			...
			case 'addexamtime': //增加考试时间5分钟 by yu 2015.12.10
			$sessionid = $this->ev->get('examsessionid');
			$ret = $this->exam->addExamtimeBySessionid($sessionid);
			if($ret==true)
			$message = array(
				'statusCode' => 200,
				"message" => "延长时间成功",
	...
	public function questions()
	{
		...
		switch($subaction)
		{
			...
			{	set_time_limit(0); //设置不超时!
				...
				$subject = $this->ev->get('subject');
				if($subject>0)
				{
				//$knowsid = trim($this->ev->get('knowsid'));
				$this->exam->importQuestionBat($uploadfile,intval($subject));
				$message = array(
					'statusCode' => 200,
					"message" => "操作成功",
					"callbackType" => "forward",
