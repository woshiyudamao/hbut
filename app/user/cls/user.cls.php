...
class user_user
{
	...
	{
		$data = array(false,array('user','user_group'),array(array('AND',"user.usergroupid = :id",'id',$id),array('AND','user.usergroupid = user_group.groupid')));
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetch($sql,array('userinfo','groupright'));
	}
	...
	public function getUserByUserName($username)
	{
        ...
        //设置用户上次登录时间
        public function setUserLoginTimeByUserName($username)
	{
		//$username = urldecode($username);
                $args['userlogtime']=TIME;
                $data = array('user',$args,array(array('AND',"username = :username",'username',$username)));
		$sql = $this->pdosql->makeUpdate($data);
		return $this->db->exec($sql);
	}
	...
	public function getUserGroupsASC()
	{
		$data = array(false,'user_group',1,false,'groupid ASC',false);
		$sql = $this->pdosql->makeSelect($data);
		return $this->db->fetchAll($sql,'groupid','groupright');
	}
	...
	{
		$data = array(false,'user_group',array(array('AND',"groupname = '".$args['groupname']."'"),array('AND',"groupmoduleid = ".$args['groupmoduleid'])));
		$sql = $this->pdosql->makeSelect($data);
		$ret=array();
		$ret=$this->db->fetch($sql);
		//如果检测到不为空说明改用户角色存在 直接返回他的ID 2015.12.1 by yu
		if($ret['groupid']) return $ret['groupid'];
